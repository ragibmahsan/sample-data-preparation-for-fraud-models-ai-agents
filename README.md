...
# Architecture
![Architecture Diagram](images/data-preparation-arch-dia.png)

## WebSocket Architecture

The system has been updated to use **WebSocket API Gateway** for real-time streaming communication, replacing the previous REST API approach for chat functionality. This provides several key benefits:

### Key Changes

1. **WebSocket API Gateway**
   - **Before**: REST API Gateway with synchronous request/response pattern
   - **After**: WebSocket API Gateway with real-time bidirectional communication
   - **Benefits**: Real-time streaming responses, better user experience, reduced latency, persistent connections

2. **New WebSocket Lambda Handlers**
...
3. **Real-time Streaming Capabilities**
   ...
   - **After**: Chunked streaming responses as they're generated by Bedrock
   - **Benefits**: Progressive content delivery, better perceived performance, real-time trace information

4. **Unified Message Routing**
   - **Before**: Separate REST endpoints for different operations
   - **After**: Message-based routing through single WebSocket connection
...
4. **Configure Environment Variables**
   After the stack deployment completes, you'll receive various outputs from CloudFormation. Update the `.env` file in the chatbot directory with these values:
   ```
